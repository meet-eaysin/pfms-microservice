...
export function calculateRemainingBalance(
  ...
  const numerator = emiAmount * (Math.pow(1 + emiRate, emiRate) - Math.pow(1 + emiRate, emisPaid));
 ...
 */
export function calculateDiversificationScore(allocations: { percentage: number }[]): number {
 ...
 */
export function calculateProjectedMonthlyAverage(spent: number, daysPassed: number): number {
 ...
 */
export function calculatePercentageAmount(percentage: number, total: number): number {
 ...
 */
export function roundToN(value: number, n: number = 2): number {
...
export function formatPercentage(value: number, decimals: number = 2): string {
  return `${roundToN(value, decimals)}%`;
}
